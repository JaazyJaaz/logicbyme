#define IR_IN 4        // infrared receive  
...
void remote_deal(void)// Implement the decoding function
{
  ...
  Serial.println();  //*
  String command = String(adrL_code, HEX); //*
  Serial.println(command); //*
  ////////////**COMMAND CODES FOR REMOTE**///////////////////////////
  if (command == "45") {            //power button
    digitalWrite(ledPin, LOW);
  }
  if (command == "c") {             //#1 button
    digitalWrite(ledPin, HIGH);
  }
  if (command == "18") {            //#2 button
    digitalWrite(ledPin, LOW);
  }
  if (command == "5e") {            //#3 button
    digitalWrite(ledPin, LOW);
  }
  if (command == "8") {             //#4 button
    digitalWrite(ledPin, HIGH);
  }
  if (command == "1c") {            //#5 button
    digitalWrite(ledPin, LOW);
  }
  if (command == "5a") {            //#6 button
    digitalWrite(ledPin, LOW);
  }
  if (command == "42") {            //#7 button
    digitalWrite(ledPin, HIGH);
  }
  if (command == "52") {            //#8 button
    digitalWrite(ledPin, LOW);
  }
  if (command == "4a") {            //#9 button
    digitalWrite(ledPin, LOW);
  }
  ///////////////////////////////////////////////////////////////////
}//remote_deal()
